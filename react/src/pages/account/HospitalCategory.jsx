...
import LocationAndAmountSetting from './LocationAndAmountSetting'; // LocationAndAmountSetting 컴포넌트 추가
...
const HospitalCategory = ({ selectedCategory, selectedItem, setSelectedCategory, setSelectedItem, setSelectedPrice, setItemSelected }) => {
    ...
    const categories = {
        '안과': [
            { name: '라식', averagePrice: 2000000 },
            { name: '라섹', averagePrice: 1800000 },
            { name: '스마일라식', averagePrice: 2200000 },
            { name: '렌즈삽입술', averagePrice: 3500000 }
        ],
        '치과': [
                        ...
                        <div className="flex space-x-6">
                            <motion.div
                                className={`w-1/2 aspect-square border p-6 rounded-lg transition ease-in-out hover:shadow-md hover:-translate-y-1 hover:scale-105 cursor-pointer ${
                                    selectedCategory === '안과' ? 'bg-green-100 scale-105' : ''
                                }`}
                                onClick={() => setSelectedCategory('안과')}
                            >
                                <h2 className="text-2xl font-bold mb-10">안과</h2>
                                <div className="flex justify-center">
                                    <img src={Eye} className="w-36" alt="안과" />
                                </div>
                            </motion.div>
                            <motion.div
                                className={`w-1/2 aspect-square border p-6 rounded-lg transition ease-in-out hover:shadow-md hover:-translate-y-1 hover:scale-105 cursor-pointer ${
